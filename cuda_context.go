...
func (d Device) MakeContext(flags ContextFlags) (CUContext, error) {
	...
	cctx := CUContext(uintptr(unsafe.Pointer(ctx)))
...
func (ctx CUContext) APIVersion() (int, error) {
	var v C.uint
	if err := result(C.cuCtxGetApiVersion(ctx.c(), &v)); err != nil {
		return -1, err
	}
	return int(v), nil
}
...
func CurrentContext() (CUContext, error) {
	...
	return CUContext(uintptr(unsafe.Pointer(ctx))), nil
}
...
func SetCurrent(ctx CUContext) error {
	return result(C.cuCtxSetCurrent(ctx.c()))
}
...
//
// CUContext creation will fail with error `UnknownError` if the compute mode of the device is CU_COMPUTEMODE_PROHIBITED.
